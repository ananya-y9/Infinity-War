...
{ 
    public static void main (String [] args) 
    {
        ...
        //String [] vNames = new String[nV]; //array of vertices

        HashMap<String, String> map = new HashMap<>();

        for (int i = 0; i < nV; i++) //make adjacency list
        {
            String vName = StdIn.readString();
            map.put(vName, null);
        }

        int nE = StdIn.readInt();
        for (int i = 0; i < nE; i++)
        {
        ...
        {
            if (map.get(key) == null) //there is no "to" from this "from", so it has an outdegree of 0
            {
                infinity = key; 
            }
        }
        ...
        for (String key : map.keySet()) //search for the values that have a "to" of infinity
        {
            if (map.get(key)!=null) //not infinity (for null pointer exception)
            {
                if (map.get(key).equals(infinity))
                {
                    StdOut.println(key);
                }
            }
